<template>
  <section class="text-center my-20 w-full">
    <div class="flex flex-col justify-center items-center mx-auto w-11/12">
      <p class="text-white text-md sm:text-lg font-light mb-8">
        Trusted by teams at over <span class="font-normal">1,000</span> of the
        world's leading organizations
      </p>
      <div
        class="flex flex-col sm:flex-wrap sm:flex-row gap-16 sm:gap-8 sm:flex-1 items-center sm:max-w-4xl justify-center"
      >
        <img
          src="../statics/logos/logo-dell.svg"
          alt="Dell"
          class="mx-auto w-full sm:w-16"
        />
        <img
          src="../statics/logos/logo-zendesk.svg"
          alt="Zendesk"
          class="mx-auto w-full sm:w-24"
        />
        <img
          src="../statics/logos/logo-rakuten.svg"
          alt="Rakuten"
          class="mx-auto w-full sm:w-16"
        />
        <img
          src="../statics/logos/logo-pacific-funds.svg"
          alt="Pacific Funds"
          class="mx-auto w-full sm:w-24"
        />
        <img
          src="../statics/logos/logo-ncr.svg"
          alt="NCR"
          class="mx-auto w-full sm:w-24"
        />
        <img
          src="../statics/logos/logo-lattice.svg"
          alt="Lattice"
          class="mx-auto w-full sm:w-24"
        />
        <img
          src="../statics/logos/logo-ted.svg"
          alt="TED"
          class="mx-auto w-full sm:w-16"
        />
      </div>
    </div>
  </section>
</template>
